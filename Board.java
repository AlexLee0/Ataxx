...
class Board {
    ...
    void clear() {
        ...
        for (char colCounter = 'a' - 2; colCounter <= 'g' + 2; colCounter++) {
            for (char rowCounter = '1' - 2;
                 rowCounter < '7' + 2; rowCounter++) {
                if (colCounter < 'a' || colCounter > 'g'
                        || rowCounter < '1' || rowCounter > '7') {
                    unrecordedSet(colCounter, rowCounter, BLOCKED);
                    incrPieces(BLOCKED, 1);
                } else if (('a' < colCounter && colCounter < 'g')
                        || ('1' < rowCounter && rowCounter < '7')) {
                    unrecordedSet(colCounter, rowCounter, EMPTY);
                    incrPieces(EMPTY, 1);
                }
            }
        }
    ...
    PieceColor getWinner() {
        if (!canMove(RED) && canMove(BLUE)
                && _numPieces[2] > 2 && _numPieces[3] > 2) {
            _winner = BLUE;
        } else if (!canMove(BLUE) && canMove(RED)
                && _numPieces[3] >= 2 && _numPieces[2] >= 2) {
            _winner = RED;
        } else if (_numPieces[2] > 2 && _numPieces[3] == 0) {
            _winner = RED;
            return _winner;
        } else if (_numPieces[3] > 2 && _numPieces[2] == 0) {
            _winner = BLUE;
            return _winner;
        } else if (!canMove(RED) && !canMove(BLUE)) {
            if (_numPieces[2] > _numPieces[3]) {
                _winner = RED;
            } else if (_numPieces[3] > _numPieces[2]) {
                _winner = BLUE;
            } else {
                _winner = EMPTY;
            }
    ...
    boolean legalMove(Move move) {
        if (move == null || getWinner() != null) {
            ...
            if (get(move.col1(), move.row1()) == EMPTY
                    && _whoseMove == get(move.col0(), move.row0())) {
                return true;
            }
        } else if (move.isPass()) {
    ...
    boolean canMove(PieceColor who) {
        for (char colCounter = 'a'; colCounter <= 'g'; colCounter++) {
            for (char rowCounter = '1'; rowCounter <= '7'; rowCounter++) {
                if (_board[index(colCounter, rowCounter)] == who) {
                    for (int boundColCounter = colCounter - 2;
                         boundColCounter <= colCounter + 2; boundColCounter++) {
                        for (int boundRowCounter = rowCounter - 2;
                             boundRowCounter <= rowCounter + 2;
                             boundRowCounter++) {
                            if (_board[index((char) boundColCounter,
                                    (char) boundRowCounter)] == EMPTY) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        if (move.isExtend()) {
            ...
            incrPieces(_board[index(move.col0(), move.row0())], 1);
            for (int colCounter = move.col1() - 1;
                 colCounter <= move.col1() + 1; colCounter++) {
                for (int rowCounter = move.row1() - 1;
                     rowCounter <= move.row1() + 1; rowCounter++) {
                    if (_board[index((char) colCounter,
                            (char) rowCounter)] == opponent) {
                        set((char) colCounter, (char) rowCounter, _whoseMove);
    ...
    void undo() {
        while (_undoSquares.peek() != null && !_undoSquares.empty()) {
            int location = _undoSquares.pop();
            PieceColor color = _undoPieces.pop();
            if (color != get(location)) {
                incrPieces(color, 1);
                incrPieces(get(location), -1);
            }
            if (color == EMPTY) {
                if (_allMoves.get(_allMoves.size() - 1).isJump()) {
                    _numJumps -= 1;
                    _board[location] = EMPTY;
                }
    ...
    boolean legalBlock(char c, char r) {
        ...
        if (_numPieces[2] + _numPieces[3] != 4) {
            return false;
        }
        return true;
    }
    ...
    void setBlock(char c, char r) {
        ...
        if (_board[index(c, r)] == EMPTY) {
            if (index(c, r) == index('d', '4')) {
                ...
                unrecordedSet(c, r, BLOCKED);
                unrecordedSet((char) ('d' - c + 'd'),
                        (char) ('4' - r + '4'), BLOCKED);
                unrecordedSet(c, (char) ('4' - r + '4'), BLOCKED);
                unrecordedSet((char) ('d' - c + 'd'), r, BLOCKED);
     ...
     */
    int totalOpen() {
        return EXTENDED_SIDE * EXTENDED_SIDE - _numPieces[1];
    }
